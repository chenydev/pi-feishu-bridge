...
test("斜杠命令帮助完整列出 V2 支持的命令和用途", () => {
	...
	for (const command of ["/help", "/commands", "/new", "/stop", "/steer <内容>", "/queue <内容>", "/q", "/compact", "/model", "/feishu status", "/feishu doctor", "/feishu policy"]) {
		assert.match(help, new RegExp(command.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
	}
	assert.match(help, /普通消息等价于 steer/);
	assert.equal(FEISHU_SLASH_COMMANDS.length, 10);
});
